...
\textbf{Cumulative radius budget:} Let $c_k$ denote the (complex) $k$-th Fourier coefficient, and define the odd-harmonic magnitudes $r_m := |c_{2m-1}|$. In the square-wave case, this gives $r_m = 4/(\pi(2m-1))$ and $c_{2m} = 0$. Define
\[
R(N) = \sum_{m=1}^{N} r_m.
\]
This quantity measures the total "circle-length budget" consumed by the first $N$ nonzero (odd) harmonics.
...
\[
\text{score}(N) = \frac{|\Delta_{N/2}| + |\Delta_N| + |\Delta_{2N}|}{3 \,\text{plateau amplitude}}.
\]
...
\subsection{Sample Console Output}

The verification script typically produces a table summarizing, for each truncation level $N$, the computational budget, relative spacing $\Delta$, overshoot factor, normalized error, effective error-zone width, and an indicator of whether all jumps were detected. One representative column layout is:
\begin{verbatim}
N | Budget | Δ/double | Overshoot | Err/jump | E-zone | Jumps?
\end{verbatim}
An example row for $N=100$ (values and labels are illustrative) is:
\begin{verbatim}
100 | 3.566 | 0.4408 | 1.089487 | 0.08949 | 0.8947 | True (0.4631)
\end{verbatim}
Note: both the exact column headers and the printed numerical values may vary slightly across versions of \texttt{gibbs\_invariant.py} and platforms. In particular, digit-level variability ($\pm 0.0002$) may occur due to BLAS or math library implementations, and later script versions may rename or reorder columns while preserving their underlying meaning.
...
Near a jump at $x = x_j$, the Dirichlet kernel $D_N(x - x_j)$ scales as $N \sin(N(x - x_j)) / \sin((x - x_j)/2)$. For $|x - x_j| \sim 1/N$, the numerator oscillates with amplitude $N$ while the denominator remains $O(1)$, so the \emph{local} squared error density in a Gibbs zone is amplified to order $O(N)$ over an interval of width $O(1/N)$.

Away from jumps, the signal is smooth, and high-frequency harmonics contribute rapidly oscillating but \emph{bounded} partial-sum errors. Pointwise, the error decays in $N$, but there is a fixed background level of squared error of order $O(1)$ when averaged over regions of size $O(1)$ that do not intersect the jump zones, yielding an $O(1)$ contribution from the complement.

Splitting the domain into $\Omega_N(\alpha)$ (total measure $O(|J|/N)$) and its complement, the heuristic scaling is:
\[
\int_{\Omega_N(\alpha)} e_N^2 \sim |J| \cdot \frac{1}{N} \cdot N^2 \sim c_{\text{zone}}(\alpha) \cdot |J|,
\]
\[
\int_{\Torus \setminus \Omega_N(\alpha)} e_N^2 \sim 2\pi \cdot c_{\text{bulk}}(\alpha),
\]
where $c_{\text{zone}}(\alpha), c_{\text{bulk}}(\alpha) \in (0,\infty)$ encode the detailed jump structure and the choice of $\alpha$. Thus both contributions remain $O(1)$ as $N \to \infty$, and the ratio
\[
F_N(\alpha) \;=\; \frac{\int_{\Omega_N(\alpha)} e_N^2}{\int_{\Torus} e_N^2}
\]
converges to a nontrivial limit $C(\alpha) \in (0,1)$, consistent with Theorem~1 and the numerical value $\approx 0.89$ observed in experiments.
